...
impl mlua::UserData for Matcher {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        methods.add_method_mut("set_pattern",|_, this: &mut Matcher, (patt,): (LuaString,)| {
        ...
        methods.add_method_mut("match", |_, this: &mut Matcher, (str,): (LuaString,)| {
            if this.pattern.as_ref().unwrap().atoms.len()==0 {return Ok((1,vec![],))};
            let b = str.as_bytes();
            let u = nucleo_matcher::Utf32Str::Ascii(b);
            let mut v = vec![];
            if let Some(score) = this.pattern.as_ref().unwrap().indices(u, &mut this.matcher, &mut v) {
...
fn nucleo_nvim(lua: &Lua) -> LuaResult<LuaTable> {
    ...
    exports.set("create_matcher",lua.create_function(|_,()|{
        let m=Matcher{
